...
float bgm_gain = 1.0f;
float se_gain = 1.0f;
ALLEGRO_SAMPLE *bgm_sp;
ALLEGRO_SAMPLE_INSTANCE *bgm_spi;
...
void load_resource() {
    ...
    load_sound("res/sound/8_queens_bgm.wav", &bgm_sp, &bgm_spi, ALLEGRO_PLAYMODE_LOOP);
...
void set_bgm_gain(float gain) {
    bgm_gain = gain;
    al_set_sample_instance_gain(bgm_spi, gain);
}
